...
def ger_submodular2(similarity, label, unlabel, uncertainty_sample, sel_num=10):
	...
	# 	print(add_ele)
	#
	# 	print(a.append(add_ele))
	# 	print(similarity[unlabel][:, label + sel_indices + a.append(add_ele)].shape)
	# 	add_max =
	...
	for i in range(sel_num):
		...
		for j in uncertainty_sample:
			...
			cur_sel = []
			cur_sel.append(j)
			add_max = np.max(similarity[unlabel][:, label + sel_indices + cur_sel], axis=1)
			gain = np.sum(add_max - unlabel_max)
